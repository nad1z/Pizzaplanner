  ...
  return (
    <div>
      <div style={{ fontSize: 10, color: '#f5e6c870', marginBottom: 6, display: 'flex', justifyContent: 'space-between' }}>
        <span className="uppercase tracking-widest">Hydration range</span>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#f5e6c860', marginTop: 4 }}>
        <span>{min}%</span>
        <span style={{ color: '#c0522aaa' }}>optimal {defaultVal}%{current !== undefined ? ` · yours ${current}%` : ''}</span>
        <span>{max}%</span>
      </div>
    </div>
  );
